...
import json, requests, numpy as np, env
...
class Payroll(models.Model):
	...
	cune = models.CharField(max_length = 150, null = True, blank= True)
	...
	def basic_payroll(cls, data):
		...
		payroll = {
		    ...
		    "payment": {
		        ...
		        "bank_name": payment_form_employee.bank_name.name if payment_form_employee.bank_name is not None else None,
	...
	def send_basic_payroll(payroll, employee, type_document, accrued, deductions, total_accrued, total_deductions):
		...
		try:
			...
			token = employee.branch.company.token
			headers = {
			  'Content-Type': 'application/json',
			  'accept': 'application/json',
			  'Authorization': f'Bearer {token}'
			}
			url = f'{env.URL_API}payroll/not_test_set_id' if type_document == 9 else f'{env.URL_API}payroll-adjust-note/not_test_set_id'
			response = requests.request("POST", url, headers=headers, data=payroll_json)
			...
			print(response_dict['cune'])
			payroll['cune'] = response_dict['cune']
			payroll_instance = Payroll.objects.create(data_payroll = payroll, employee=employee, was_sent = True, cune = response_dict['cune'])
